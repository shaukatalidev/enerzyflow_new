...
// ✅ Helper functions outside component (no re-creation on render)
...
const OrderCard = memo(({ order, onClick }: { order: Order; onClick: () => void }) => {
  ...
  // ✅ Custom comparison - only re-render if order data changes
  return (
...
const EmptyState = memo(({ title, description, icon }: {
  title: string;
  description: string;
...
export default function MainDashboard() {
  ...
  const categorizedOrders = useMemo(() => {
    ...
    const rejected = orders.filter(
      (order) => order.status === "declined" || order.payment_status === "payment_rejected"
    );

    // Sort active: payment pending first
    active.sort((a, b) => {
    ...
    // Sort others by date
    rejected.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    completed.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

    return {
      active: { all: active, displayed: active.slice(0, 3) },
      rejected: { all: rejected, displayed: rejected.slice(0, 3) },
      completed: { all: completed, displayed: completed.slice(0, 3) },
    };
  }, [orders]);
  ...
  const handleOrderClick = useCallback((orderId: string) => {
    ...
    if (order.payment_status === "payment_pending" || order.payment_status === "payment_rejected") {
      setUiState(prev => ({ ...prev, showPaymentAlert: true }));
      setTimeout(() => {
        router.push(`/order/${orderId}/invoice`);
        setUiState(prev => ({ ...prev, showPaymentAlert: false }));
      }, 1500);
      return;
    }
  ...
  const handleNewOrder = useCallback(() => {
    setUiState(prev => ({ ...prev, isCreatingOrder: true }));
    router.push("/order");
  }, [router]);

  const handleViewAllActive = useCallback(() => {
    setModalState({ isOpen: true, orderType: "active" });
  }, []);

  const handleViewAllRejected = useCallback(() => {
    setModalState({ isOpen: true, orderType: "rejected" });
  }, []);

  const handleViewAllCompleted = useCallback(() => {
    setModalState({ isOpen: true, orderType: "completed" });
  }, []);
    ...
    <>
      <div className="min-h-screen bg-gray-50">
        <div className="flex">
          ...
          >
            ...
            <nav className="mt-8 px-4 space-y-2">
              <a href="#" className="flex items-center px-4 py-2 text-sm font-medium text-blue-600 bg-blue-50 rounded-md cursor-pointer">
          ...
          <div className="flex-1 lg:ml-0">
            <div className="p-4 lg:p-6">
              ...
              {uiState.error && (
                <div className="mb-6 bg-red-50 border-l-4 border-red-400 p-4 rounded-r-lg">
                  <div className="flex">
                    ...
                    <p className="ml-3 text-sm text-red-700">{uiState.error}</p>
                  </div>
                </div>
              )}

              {/* Payment Alert */}
              {uiState.showPaymentAlert && (
                <div className="mb-6 bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r-lg animate-pulse">
                  <div className="flex items-start">
                    ...
                    <div className="ml-3 flex-1">
                      <p className="text-sm font-medium text-amber-800">Payment screenshot required</p>
                      <p className="text-sm text-amber-700 mt-1">Please upload payment screenshot to continue. Redirecting...</p>
                    </div>
                    <button onClick={handleCloseAlert} className="ml-auto flex-shrink-0 text-amber-600 hover:text-amber-800 cursor-pointer">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                  <div className="text-center">
                    ...
                    <p className="text-3xl font-bold text-gray-900 mb-2">{categorizedOrders.active.all.length}</p>
                ...
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                  <div className="text-center">
                    ...
                    <p className="text-3xl font-bold text-gray-900 mb-2">{categorizedOrders.rejected.all.length}</p>
                ...
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                  <div className="text-center">
                    ...
                    <p className="text-3xl font-bold text-gray-900 mb-2">{categorizedOrders.completed.all.length}</p>
              ...
              <button
                ...
                disabled={uiState.isCreatingOrder}
              ...
              <div className="space-y-6">
                ...
                <section>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {categorizedOrders.active.all.length > 3 && (
                      <button onClick={handleViewAllActive} className="text-sm text-blue-600 hover:text-blue-700 font-medium cursor-pointer">
                        View all
                      </button>
                    )}
                  </div>

                  {categorizedOrders.active.displayed.length === 0 ? (
                    <EmptyState
                    ...
                    <div className="space-y-3">
                      {categorizedOrders.active.displayed.map((order) => (
                        <OrderCard key={order.order_id} order={order} onClick={() => handleOrderClick(order.order_id)} />
                      ))}
                    </div>
                  )}
                </section>
                ...
                {/* Rejected Orders */}
                {categorizedOrders.rejected.all.length > 0 && (
                  <section>
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-lg font-semibold text-gray-900">Rejected Orders</h2>
                      {categorizedOrders.rejected.all.length > 3 && (
                        <button onClick={handleViewAllRejected} className="text-sm text-blue-600 hover:text-blue-700 font-medium cursor-pointer">
                          View all
                        </button>
                      )}
                    </div>
                    ...
                    <div className="space-y-3">
                      {categorizedOrders.rejected.displayed.map((order) => (
                        <OrderCard key={order.order_id} order={order} onClick={() => handleOrderClick(order.order_id)} />
                      ))}
                    </div>
                  </section>
                ...
                <section>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {categorizedOrders.completed.all.length > 3 && (
                      <button onClick={handleViewAllCompleted} className="text-sm text-blue-600 hover:text-blue-700 font-medium cursor-pointer">
                        View all
                      </button>
                    )}
                  </div>

                  {categorizedOrders.completed.displayed.length === 0 ? (
                    <EmptyState
                    ...
                    <div className="space-y-3">
                      {categorizedOrders.completed.displayed.map((order) => (
                        <OrderCard key={order.order_id} order={order} onClick={() => handleOrderClick(order.order_id)} />
                      ))}
                    </div>
                  )}
        ...
        {uiState.isSidebarOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-40 lg:hidden" onClick={handleCloseSidebar} />
        )}
      </div>
      ...
      <OrdersModal
        ...
        orderType={modalState.orderType}
