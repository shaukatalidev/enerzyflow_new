...
export default function MainDashboard() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // Separate active and completed orders based on status
  ...
  const getProductName = (order: Order) => {
    // Construct product name from order details
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Mobile Header - Only show on mobile since main header handles desktop */}
              ...
              <div>
                ...
                {activeOrders.length === 0 ? (
                  <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center">
                    ...
                    <p className="text-gray-500 font-medium mb-2">No active orders</p>
                  ...
                  <div className="space-y-3">
                    {activeOrders.slice(0, 3).map((order) => (
                      <button
                        key={order.order_id}
                        onClick={() => router.push(`/order/status?orderId=${order.order_id}`)}
                          ...
                          <div className="flex items-start space-x-4">
                            <div className="w-12 h-12 bg-black rounded-xl flex items-center justify-center flex-shrink-0">
                              {order.label_url ? (
                                ...
                                <span className="text-white text-xs font-bold">spicy</span>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center justify-between">
                                <span className={`text-xs font-medium px-2 py-1 rounded-full ${getStatusColor(order.status)}`}>
                                  {getStatusCapitalized(order.status)}
                                </span>
              ...
              <div>
                ...
                {completedOrders.length === 0 ? (
                  <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center">
                    ...
                    <p className="text-gray-500 font-medium mb-2">No completed orders</p>
