  ...
  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
    ...
    <div className="space-y-3 bg-gray-50 rounded-xl p-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <span
          className={`text-sm font-medium px-2 py-1 rounded-full ${orderService.getPaymentStatusColor(
          ...
          <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-3">
            <div className="flex items-start space-x-2">
              ...
              <p className="text-sm text-amber-800">
                <strong>Required:</strong> You must upload payment screenshot to
                track your order
              </p>
            </div>
          </div>
...
export default function InvoiceDownloadPage() {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        if (
          response.order.payment_status === "payment_uploaded" ||
  ...
  const handleUploadPayment = useCallback(async () => {
    ...
    try {
      const response = await orderService.uploadPaymentScreenshot(
        orderId,
        paymentFile
      );
      setUploadSuccess(true);
